    ...
    public partial class FrmSony_New : Form
    {
        ...
        private void txtdo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                conn.Open();
                comm.CommandText = "Select CG,DO,DODate,ShipToParty,ShipToName,ShipToStreet,Province,GrossWeight,Quatity,DeliveryDate,RealWeight,Remark,ProvinceID from tb_sonyplan where DO = '"+ txtdo.Text.Trim() +"' and [DODate] = CDate('" + dtppgi.Value.ToString("dd/MM/yyyy 00:00:00") + "')";
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (flag == true)
                    {
                        ...
                        {
                            string[] row = new string[] { NullValue(rowds["CG"].ToString()), NullValue(rowds["DO"].ToString()), NullValue(rowds["DODate"].ToString()), NullValue(rowds["ShipToParty"].ToString()), NullValue(rowds["ShipToName"].ToString()), NullValue(rowds["ShipToStreet"].ToString()), rowds["Province"].ToString(), NullValue(rowds["GrossWeight"].ToString()), rowds["Quatity"].ToString(), NullValue(rowds["DeliveryDate"].ToString()), rowds["RealWeight"].ToString(), rowds["Remark"].ToString(), NullValue(rowds["ProvinceID"].ToString()) };
                    ...
                    txtkhachhang.Text = NullValue(ds.Tables[0].Rows[0]["ShipToName"].ToString());
                    txtdiachikh.Text = NullValue(ds.Tables[0].Rows[0]["ShipToStreet"].ToString());
