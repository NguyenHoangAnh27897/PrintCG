...
namespace PrintCG_24062016
{
    public partial class FrmSony : Form
    {
        ...
        private void FrmSony_Load(object sender, EventArgs e)
        {
            ...
            if (txtmaxcg.Text == "")
            {
               // txtmaxcg.Text = "SN" + nam + thang + ngay + "0001";
                txtmaxcg.Text = "SN" + nam + thang + ngay + txttuso.Text;
            }
            if (txtmaxcg.Text.Substring(2, 6) != nam + thang + ngay)
            {
                //txtmaxcg.Text = "SN" + nam + thang + ngay + "0001";
                txtmaxcg.Text = "SN" + nam + thang + ngay + txttuso.Text;
            }
        ...
        private void load_settings()
        {
           ...
           // txtngaygui.Text = (String)Application.UserAppDataRegistry.GetValue("frmsony.txtngaygui", String.Empty);
            ...
            txtmaxcg.Text = (String)Application.UserAppDataRegistry.GetValue("frmsony.txtmaxcg", String.Empty);
            txttuso.Text = (String)Application.UserAppDataRegistry.GetValue("frmsony.txttuso", String.Empty);
            txtdenso.Text = (String)Application.UserAppDataRegistry.GetValue("frmsony.txtdenso", String.Empty);
        ...
        private void save_settings()
        {
           ...
           // Application.UserAppDataRegistry.SetValue("frmsony.txtngaygui", txtngaygui.Text);
            ...
            Application.UserAppDataRegistry.SetValue("frmsony.txtmaxcg", txtmaxcg.Text);
            Application.UserAppDataRegistry.SetValue("frmsony.txttuso", txttuso.Text);
            Application.UserAppDataRegistry.SetValue("frmsony.txtdenso", txtdenso.Text);
        ...
        private DataTable ReadExcelFile(string sheetName, string path)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection())
            {
                ...
                using (OleDbCommand comm = new OleDbCommand())
                {
                    comm.CommandText = "Select [CG],[SL],[TL],[HD],[TP]," + cmbdo.Text + "," + cmbnguoinhan.Text + "," + cmbdiachi.Text + "," + cmbsoluong.Text + "  from [" + sheetName + "] where [" + cmbdo.Text + "] like '%" + txtdo.Text + "%'";
                     //comm.CommandText = "Select * from [" + sheetName + "]";
        ...
        private void btnin_Click(object sender, EventArgs e)
        {
            ...
            if (int.Parse(txtdenso.Text) < int.Parse(txtmaxcg.Text.Substring(8, 4)))
            {
                MessageBox.Show("Đã hết giới hạn số phiếu in ngày hôm nay!");
            }
            else if (int.Parse(txtdenso.Text) >= int.Parse(txtmaxcg.Text.Substring(8, 4)))
            {
                bool flag = false;
        ...
        private void updateexcel()
        {
            ...
            int numberOfRecords;
            if (txthoadon.Text != "" && txttrongluongtong.Text != "" && cmbtp.Text.Trim() != "")
            {
                sokien = "";
                using (OleDbConnection conn = new OleDbConnection())
                {
                    ...
                    using (OleDbCommand comm = new OleDbCommand())
                    {
                        comm.CommandText = "Update [" + cmbsheet.Text + "] set CG = '" + txtmaxcg.Text + "',HD = '" + txthoadon.Text + "',SL ='" + txtsoluong.Text + "',TL ='" + txttrongluongtong.Text + "',TP ='" + cmbtp.Text + "'  where [" + cmbdo.Text + "] like '%" + txtdo.Text + "%'";
                        comm.Connection = conn;
                        conn.Open();
        ...
        private void inphieugui()
        {
            ...
            else if (rdba5.Checked == true)
            {
                ...
                _txttp.Text = cmbtp.SelectedValue.ToString();
                TextObject _txthd = (TextObject)rpt.ReportDefinition.Sections["Section3"].ReportObjects["txthd"];
                _txthd.Text = txthoadon.Text; ;
                ...
                string bef = txtmaxcg.Text.Substring(0, 8);
                int aft = int.Parse(txtmaxcg.Text.Substring(8, 4)) + 1;
                string new_aft = "";
        ...
        {
            if (txttuso.Enabled == false)
            {
                txttuso.Enabled = true;
                txtdenso.Enabled = true;
                txtmaxcg.Enabled = true;
            }else if(txttuso.Enabled == true)
            {
